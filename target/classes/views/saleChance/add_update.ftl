...
<html>
    ...
    <body class="childrenBody">
        <form class="layui-form" style="width:80%;">
            <#-- 设置id的隐藏域 -->
            <input type="hidden" name="id" value="${(saleChance.id)!}"><#--传入id 后台进行查询，把返回值设置到域对象当中，然后前台获取，这样更新界面就会有数据 -->
            <#-- 设置指派人 -->
            <input type="hidden" id="saleChanceName" value="${(saleChance.assignMan)!}"><#-- 为了让js获取到值，与原本下拉框中所便利的值进行比较，相同就会被显示出来 -->
            ...
            <div class="layui-form-item layui-row layui-col-xs12">
                ...
                <div class="layui-input-block">
                    <select name="assignMan" id="assignMan">
                        <option value="${(saleChance.cgjl)!}">请选择</option>
                    </select>
                </div>
            </div>
            <br/>
